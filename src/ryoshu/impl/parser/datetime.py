...
from ryoshu.impl.parser import builtins as builtins_parsers
    ...
    def __init__(
        ...
        *,
        ...
        if resolution < 1 and resolution not in _VALID_BASE_10:
            ...
            message = f"Resolutions smaller than 1 must be a power of 10, got {resolution}."
            raise ValueError(message)
    ...
    def dumps(self, argument: datetime.datetime) -> str:
        ...
        if self.strict:
            if not argument.tzinfo:
                message = "Strict DatetimeParsers can only load timezone-aware datetimes."
                raise ValueError(message)
        else:
...
class TimeParser(parser_base.Parser[datetime.time]):
    ...
    def dumps(self, argument: datetime.time) -> str:
        ...
        if self.strict:
            if not argument.tzinfo:
                message = "Strict TimeParsers can only load timezone-aware times."
                raise ValueError(message)
        else:
            argument = argument.replace(tzinfo=self.timezone)
